...
const CLIENT_ID = process.env.CLIENT_ID || '441fc6e5-a564-4ba4-a302-64d388659170';
...
async function initializeWhatsApp() {
    try {
        ...
        client.on('disconnected', async (reason) => {
            ...
            console.log(`🔄 Auto-reconnecting Estudio Jurídico Villegas in ${reconnectDelay/1000}s (attempt ${reconnectAttempts + 1})`);
            
            setTimeout(async () => {
                try {
                    await initializeWhatsApp();
                } catch (error) {
                    console.error(`❌ Reconnection failed for Estudio Jurídico Villegas:`, error);
        ...
        client.on('auth_failure', async (msg) => {
            console.log(`❌ Auth failed for Estudio Jurídico Villegas:`, msg);
            qrCodeData = null;
            isConnected = false;
            connectedUser = null;
            isInitializing = false;
            ...
            try {
                if (fs.existsSync(sessionDir)) {
                    ...
                    console.log(`🧹 Cleared session for Estudio Jurídico Villegas`);
                }
            } catch (cleanError) {
            ...
            console.log(`🔄 Force restarting Estudio Jurídico Villegas with clean session`);
            setTimeout(() => {
                initializeWhatsApp();
            }, 5000);
        });
...
// Force restart endpoint - EMERGENCY RECOVERY
        app.get('/force-restart', async (req, res) => {
            console.log(`🚨 FORCE RESTART requested for Estudio Jurídico Villegas`);
            ...
            try {
                ...
                // Immediate restart
                console.log(`🔄 IMMEDIATE restart Estudio Jurídico Villegas...`);
                setTimeout(() => {
                    initializeWhatsApp();
                }, 2000);
                
                res.json({ success: true, message: `Force restart initiated for Estudio Jurídico Villegas` });
            } catch (error) {
