...
const PORT = process.env.CLIENT_PORT || 3007;
const FASTAPI_URL = 'http://localhost:8001';
const CLIENT_ID = process.env.CLIENT_ID || '0b060bf9-3f33-4dd4-bbb7-603dd6b48d60';

// Deploy-aware environment detection
const isDeployEnv = process.env.EMERGENT_ENV === 'deploy';
console.log(`Individual service for Gonzalo running in ${isDeployEnv ? 'DEPLOY' : 'PREVIEW'} environment`);
...
async function initializeWhatsApp() {
    try {
        ...
        isInitializing = true;
        console.log(`🚀 FAST INIT: Starting WhatsApp for client Gonzalo...`);
        ...
        client.on('qr', async (qr) => {
            console.log(`QR Code received for Gonzalo`);
            qrCodeData = qr;
            reconnectAttempts = 0;
            try {
                const qrDataUrl = await qrcode.toDataURL(qr);
                console.log(`QR Code generated successfully for Gonzalo`);
            } catch (err) {
        ...
        client.on('authenticated', () => {
            console.log(`WhatsApp authenticated successfully for Gonzalo`);
        });

        // Ready event
        client.on('ready', async () => {
            console.log(`WhatsApp client is ready for Gonzalo!`);
            isConnected = true;
            qrCodeData = null;
            ...
            try {
                ...
                console.log(`Connected user for Gonzalo:`, connectedUser);
            } catch (err) {
        ...
        client.on('message', async (message) => {
            if (!message.fromMe && message.body) {
                ...
                if (message.from.includes('-') || message.from.includes('@g.us')) {
                    console.log(`Ignored group message for Gonzalo from ${message.from}: ${message.body}`);
                    return;
                }
                
                if (!message.from.includes('@c.us')) {
                    console.log(`Ignored non-private message for Gonzalo from ${message.from}`);
                    return;
                }
                
                console.log(`Message for Gonzalo from ${message.from}: ${message.body}`);
                ...
                try {
                    ...
                    if (response.data.reply) {
                        ...
                        console.log(`Reply sent for Gonzalo:`, response.data.reply);
                    } else if (response.data.paused) {
                        console.log(`Conversation is paused for Gonzalo, no automatic reply sent`);
                    }
                } catch (error) {
                    console.error(`Error processing message for Gonzalo:`, error);
                    try {
                        await message.reply('Lo siento, hubo un error procesando tu mensaje. Por favor intenta nuevamente.');
                    } catch (replyError) {
                        console.error(`Error sending fallback message for Gonzalo:`, replyError);
                    }
                }
            }
        });
        ...
        console.log(`Starting WhatsApp client initialization for Gonzalo...`);
        await client.initialize();

    } catch (error) {
        console.error(`Error initializing WhatsApp for Gonzalo:`, error);
        isInitializing = false;
        
        if (reconnectAttempts < MAX_RECONNECT_ATTEMPTS) {
            reconnectAttempts++;
            console.log(`Initialization retry ${reconnectAttempts}/${MAX_RECONNECT_ATTEMPTS} for Gonzalo`);
...
app.get('/logout', (req, res) => {
    res.json({
        success: true,
        message: 'Logout initiated for Gonzalo',
        instructions: 'Please wait while we disconnect your device from WhatsApp.'
    });
    
    setTimeout(async () => {
        try {
            console.log(`Initiating complete WhatsApp logout for Gonzalo...`);
            
            if (client) {
                await client.logout();
                console.log(`WhatsApp logout completed for Gonzalo`);
                ...
                if (fs.existsSync(sessionDir)) {
                    console.log(`Removing all session data for Gonzalo...`);
                    fs.rmSync(sessionDir, { recursive: true, force: true });
                }
                
                console.log(`Complete logout and cleanup finished for Gonzalo`);
                client = null;
            }
        } catch (error) {
            console.error(`Error during logout for Gonzalo:`, error);
        }
    }, 1000);
});
...
const gracefulShutdown = async () => {
    console.log(`Shutting down individual service for Gonzalo...`);
    
    if (client) {
        console.log(`Destroying WhatsApp client for Gonzalo...`);
        try {
            await client.destroy();
        } catch (error) {
            console.error(`Error destroying client for Gonzalo:`, error);
        }
    }
    
    if (server) {
        console.log(`Closing HTTP server for Gonzalo...`);
        server.close(() => {
            console.log(`HTTP server closed for Gonzalo`);
            process.exit(0);
        });
        
        setTimeout(() => {
            console.log(`Forcing shutdown for Gonzalo...`);
            process.exit(1);
        }, 30000);
    } else {
        process.exit(0);
