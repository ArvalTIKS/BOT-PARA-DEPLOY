...
const CLIENT_ID = process.env.CLIENT_ID || '18d784d5-7bd6-407e-85c7-4202cf5201ed';
...
async function initializeWhatsApp() {
    try {
        ...
        isInitializing = true;
        console.log(`Initializing WhatsApp for client test...`);
        ...
        client.on('disconnected', async (reason) => {
            ...
            if (reconnectAttempts < MAX_RECONNECT_ATTEMPTS) {
                reconnectAttempts++;
                console.log(`Reconnection attempt ${reconnectAttempts}/${MAX_RECONNECT_ATTEMPTS} for test`);
        ...
        client.on('auth_failure', async (msg) => {
            console.error(`Authentication failure for test:`, msg);
            
            if (fs.existsSync(sessionDir)) {
                console.log(`Removing corrupted auth data for test`);
                try {
            ...
            setTimeout(() => {
                console.log(`Reinitializing after auth failure for test`);
                initializeWhatsApp();
            }, 10000);
        });
